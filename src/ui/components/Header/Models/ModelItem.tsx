...
export const ModelItem = (
    ...
    const onModelSelect = () => {
        ...
        globalEventBus.emit('model:change', model.value)
        // globalEventBus.emit('conversation:new') Completion will enforce this but ui should not change-no user disruption
        globalEventBus.emit('model:selector:hide')
    }
    useEffect(() => {
        const modeChangeRequest = globalEventBus.on('model:change:request', (targetModel) => {
            if (targetModel === model.value) onModelSelect()
            return () => modeChangeRequest.unsubscribe()
        })
    })
    return (
        <div
