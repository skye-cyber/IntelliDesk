...
import { normalizeMathDelimiters, normaliZeMathDisplay } from '../../MathBase/MathNormalize';
import { implementUserCopy } from '../Utils/chatUtils';
...
import { InputPurify } from "../../Utils/chatUtils";
import { scrollToBottom } from './util';
import { debounceRenderKaTeX } from '../../MathBase/mathRenderer';
import { implementUserCopy } from '../../Utils/chatUtils';
//import { handleCodeCopy } from "../../Utils/chatUtils";
    ...
    try {
        ...
        for await (const chunk of stream) {
            ...
            if (codeBuffer && codeBuffer.code) {
                ...
                if (!canvasutil.isCanvasOpen()) showCanvas();
            }
			...
			${isThinking || thinkContent ? `
				...
				${actualResponse ? `
					...
					<p style="color: #333;">${window.marked(normalizeMathDelimiters(actualResponse))}</p>
...
export async function MistraVision(text, fileType, fileDataUrl = null, modelName) {
    const _Timer = new window.Timer;
    ...
    try {
        ...
        for await (const chunk of visionstream) {
            ...
            if (codeBuffer && codeBuffer.code) {
                ...
                if (!canvasutil..isCanvasOpen()) window.openCanvas();
            }
            ...
            debounceRenderKaTeX(`.${VisionMessageUId}`, 3000, true);
        }
        ...
        // Render katex immediately
        debounceRenderKaTeX(`.${VisionMessageUId}`, null, true);
...
export function addUserMessage(text, fileType, fileDataUrl, fileContainerId) {
...
export function addLoadingAnimation(aiMessageDiv) {
