...
export const Canvas = ({ isOpen, onToggle }) => {
    ...
    const codeViewButtonRef = useRef(null)
    const handIndicator = useRef(null)
    ...
    const initialize = useCallback(() => {
        const { lineNumbers, codeScrollWrapper } = refs.current;
        ...
        canvasUpdate();
        new Caret(codeView, handIndicator.current, codeScrollWrapper, scrollState.current.currentScroll).updatePenPosition()
    ...
    const updateLineNumbers = useCallback(() => {
        const { lineNumbers, lineCounter } = refs.current;
        const codeView = codeViewRef.current
    ...
    const setActiveButton = useCallback((activeBtn) => {
        const { btnPreview } = refs.current;
        if (!codeViewButtonRef.current || !btnPreview) return;

        [codeViewButtonRef.current, btnPreview].forEach(btn => {
    ...
    const handleCodeView = useCallback(() => {
        const { codeBlockContainer, previewView } = refs.current;
        codeBlockContainer?.classList.remove('hidden');
        previewView?.classList.add('hidden');
        setActiveButton(codeViewButtonRef.current);
    }, [setActiveButton]);
    ...
    const updatePreview = useCallback(() => {
        ...
        const { previewView } = refs.current;
        const codeView = codeViewRef.current
    ...
    const handleWheelScroll = useCallback(() => {
        ...
        if (handIndicator.current && Caret) {
            new Caret(codeView, handIndicator.current, codeScrollWrapper, scrollState.current.currentScroll)
                .updatePenPosition();
        }
    ...
    const syncScroll = useCallback((e) => {
        const { lineNumbers } = refs.current;
        const codeView = codeViewRef.current
        if (!codeView || !lineNumbers) return;
    ...
    useEffect(() => {
        const openInCanvas = (code) => {
            ...
            if (!canvasutil.isCanvasOpen()) onToggle()
            setTimeout(() => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setTimeout(() => {
                canvasRef.current?.classList.remove('translate-x-[110vw]')
            }, 10)
        } else {
            canvasRef.current?.classList.add('translate-x-[110vw]')
            setTimeout(() => {
    ...
    return (
        <section
            ref={canvasRef}
            id="canvas-wrapper"
            className="w-full max-w-[45vw] bg-gradient-to-tr from-purple-100 via-purple-200 to-pink-100 dark:from-secondary-900 dark:via-blend-900 dark:to-accent-900 h-[95vh] flex items-center justify-center font-sans text-gray-800 dark:text-purple-200 border-x border-y border-t-0 border-r-0 border-blue-500 dark:border-cyan-500/0 rounded-xl rounded-b-none p-0.5 border-2 hidden translate-x-[110vw] transform-transition duration-300">

            <div aria-label="Canvas container" className="mt-0 w-full max-w-full bg-white dark:bg-primary-800 rounded-xl shadow-xl rounded-b-none flex flex-col h-full overflow-hidden ring-1 ring-purple-200 dark:ring-purple-700">
                ...
                <header className="flex items-center justify-between px-0.5 xl:px-2 py-1 border-b border-purple-200 dark:border-purple-700 select-none transition-colors duration-500">
                    ...
                    <div className='group relative'>
                        <button onClick={onToggle} className="flex justify-center items-center py-[0px] px-[2px] cursor-w-resize group">
                ...
                <section className="flex-1 w-full p-0 space-y-0 h-full overflow-hidden">
                    ...
                    <div id="code-block-container" className="flex flex-row h-full overflow-hidden ring-2 ring-purple-300 dark:ring-purple-700 shadow-inner bg-white dark:bg-zinc-950 select-text text-sm max-w-full">
                        ...
                        <div id="code-scroll-wrapper" className="flex flex-row flex-1 pl-2 overflow-hidden">
                            ...
                            <div
                                ref={handIndicator}
                                id="hand-indicator"
                                className="absolute z-[1] pointer-events-none transition-all duration-700 ease-out"
