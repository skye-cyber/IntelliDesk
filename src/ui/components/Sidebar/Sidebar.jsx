...
export const Sidebar = ({ isOpen, onToggle }) => {
    ...
    useEffect(() => {
        ...
        const newSession = globalEventBus.on('conversation:new', (temporary = false) => {
            ...
            const model = chatutil.get_multimodal_models().includes(currentModel)
                ? 'multimodal'
                : 'chat'
            clearMessages()
    ...
    return (
        ...
        >
            ...
            <section data-action="options" className='ml-1 mt-0 space-y-0.5 text-gray-800 dark:text-white'>
                <div onClick={() => globalEventBus.emit('conversation:new')} id="new-chat" className="flex items-center rounded-lg flex min-w-0 items-center gap-1.5 cursor-pointer hover:bg-gray-200 p-1.5 dark:hover:bg-blend-600">
