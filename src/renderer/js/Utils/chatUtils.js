...
renderer.code = function(code) {
    ...
    if (canvasutil.isCanvasOpen()) {
        //increament code buffer
        window.codeBuffer = { lang: validLanguage, code: `<code id="${validLanguage}" data-value=${renderButtonId} class="hljs ${validLanguage} block whitespace-pre w-full rounded-md bg-none font-mono transition-colors duration-500">${highlighted}</code>` }
...
export function handleRequestError(error, userMessage, aiMessage, VS_url = null) {
    try {
        processing = false;
