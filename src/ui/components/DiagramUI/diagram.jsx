...
export const DiagramUi = ({ isOpen, onClose, content }) => {
    ...
    const viewSidebar = useRef(null)

    const openView = useCallback(() => {
        view.current.classList.remove('hidden', 'opacity-0', 'translate-x-full');
        view.current.classList.add('opacity-100', 'translate-x-0')
    })

    const closeView = useCallback(() => {
        view.current.classList.remove('opacity-100', 'translate-x-0')
        view.current.classList.add('translate-x-full');
        setTimeout(() => {
    ...
    useEffect(() => {
        const handleEscape = (e) => {
            if (e.key === 'Escape' && isOpen) closeView();
        };
            ...
            >
                ...
                <div className="relative flex-1 flex bg-white dark:bg-slate-900">
                    ...
                    <div className="flex-1 p-6 overflow-x-hidden">
                        ...
                        <button onClick={() => viewSidebar.current.classList.toggle('hidden')} className="absolute z-10 top-0 right-0 flex justify-center items-center text-slate-900 dark:text-white py-[0px] px-[2px] cursor-w-resize group focus:ring-none focus:outline-none">
                    ...
                    <div
                        ref={viewSidebar}
                        id="dg-sidebar"
                        className="hidden md:block w-fit max-w-96 border-l border-slate-700/50 bg-slate-100 dark:bg-slate-800/30 backdrop-blur-sm flex flex-col">
