...
export const Sidebar = ({ isOpen, onToggle }) => {
    ...
    const showPanel = useCallback(() => {
        ...
        waitForElement('#conversationPane', (panel) => {
            ...
            panel.classList.remove('w-fit');
            panel.classList.add('w-80', 'max-w-[30vw]');

            waitForElement('#chat-container', (container) => {
                if (container.classList.contains('mx-24')) {
                    container.classList.replace('mx-24', 'mx-2');
                }
            });
        });
    }, [onToggle]);
    ...
    const hidePanel = useCallback(() => {
        waitForElement('#conversationPane', (panel) => {
            ...
            panel.classList.remove('w-80', 'max-w-[30vw]');
            panel.classList.add('w-fit');

            waitForElement('#chat-container', (container) => {
                if (container.classList.contains('mx-2')) {
                    container.classList.replace('mx-2', 'mx-24');
                }
            });
        });
    }, []);
    ...
    return (
        <div
            ...
            className="h-screen w-fit bg-gradient-to-b from-slate-50 to-blue-50/30 dark:from-gray-900 dark:to-slate-900/95 backdrop-blur-xl border-r border-gray-200/50 dark:border-gray-700/50 transform transition-all duration-500 ease-out overflow-hidden"
        >
            {/* Header Section */}
            <section className="sticky top-0 z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-200/50 dark:border-gray-700/50 p-1">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                        <div className=" relative w-8 h-8 bg-blue-500/0 rounded-lg flex items-center justify-center dark:shadow-lg cursor-pointer group transition-all duration-300">
                            <img src={indellidesk} className="w-8 h-8 text-gray-400 dark:text-gray-500 group-hover:hidden"></img>

                            <svg onClick={togglePanel} width="20" height="20" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg" data-rtl-flip="" className="icon max-md:hidden hidden group-hover:flex h-8 w-8 fill-primary-300 dark:fill-white cursor-w-resize group">
            ...
            <section data-action="options" className='ml-1 mt-3 space-y-2 text-gray-800 dark:text-white'>
                <div id="new-chat" className="flex items-center rounded-lg flex min-w-0 items-center gap-1.5 cursor-pointer hover:bg-gray-200 p-1.5">
                ...
                <div className='flex items-center rounded-lg flex min-w-0 items-center cursor-pointer hover:bg-gray-200 px-1.5'>
                ...
                <div className='flex items-center rounded-lg flex min-w-0 gap-1.5 items-center cursor-pointer hover:bg-gray-200 p-1.5'>
                    <svg className="text-blend-700 w-5 h-5 text-gray-500 group-hover:text-primary-700 dark:text-primary-800 dark:group-hover:text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
            ...
            {/* Conversations List */}
            <div id="conversations" className="verbose-hide hidden h-[calc(100vh-120px)] overflow-y-auto py-2 px-3 space-y-1">
                {/* Empty State */}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-white/80 to-transparent dark:from-gray-900/80 backdrop-blur-md border-t border-gray-200/50 dark:border-gray-700/50">
                <div className="flex items-center justify-between text-xs">
                    ...
                    <button onClick={() => document.getElementById('togglePane')?.click()} className='verbose-hide hidden h-6 w-6 hover:rotate-180 transition-all duration-700 cursor-w-resize'>
