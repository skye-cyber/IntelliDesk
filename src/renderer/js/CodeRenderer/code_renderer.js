...
renderer.code = function(code) {
    ...
    return `
    <div class="my-2 block bg-gray-200 dark:bg-gray-400  rounded-md transition-colors duration-100">
        <section class="flex justify-between top-1 p-1 w-full bg-gray-300 rounded-t-md dark:bg-zinc-600 box-border transition-colors duration-700">
            <!-- Language -->
        ...
        <div class="flex justify-between items-center space-x-2">
            <button onclick="window.openInCanvas(this, '${openInCanvas}')" id="openBtn" class="w-fit h-fit inline-flex items-center gap-0.5 text-xs rounded-md p-1 cursor-pointer shadow-md shadow-[rgba(16,24,40,0.06)] font-semibold text-[#0f172a] dark:text-white hover:bg-zinc-400 transition-transform transition-all duration-300" aria-pressed="false" title="Open coding canvas">
            ...
            <!-- Copy button -->
            <button id="${copyButtonId}" onclick="window.handleCodeCopy(this, '${renderButtonId}');" class="copy-button flex items-center gap-0.5 p-1 hover:bg-zinc-400 rounded-md font-semibold text-xs text-white cursor-pointer transform transition-all duration-700">
            ...
            <button id="${downloadButtonId}" onclick="${get_render_fn()}" class="run-button flex items-center gap-0.5 rounded-md text-xs text-white cursor-pointer hover:bg-zinc-400 p-1 transform transition-all duration-700">
            ...
            ${(get_render_fn() || 1===1) ? `
            <button id="${renderButtonId}" onclick="${get_render_fn()}" class="run-button flex items-center gap-0.5 rounded-md text-xs text-white cursor-pointer hover:bg-zinc-400 p-1 transform transition-all duration-700">
                <!-- Network / Diagram Icon -->
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="currentColor" viewBox="0 0 640 640">
        ...
        <div class="p-2 w-full bg-gray-200 dark:bg-blend-900 rounded-md rounded-t-none overflow-auto scrollbar-hide transition-colors duration-700">
        <code data-value=${renderButtonId} class="p-2 hljs ${validLanguage} block whitespace-pre  font-brand transition-colors duration-700 overflow-x-auto ">${highlighted}</code>
        </div>
    </div>
            `
};
