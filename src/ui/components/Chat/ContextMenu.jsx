...
export const ChatContextMenu = ({ isOpen, onClose, id }) => {
    ...
    // Enforce lock to avoid multiple context menu at same time
    if (!isOpen) return
    ...
    return (
        ...
        >
            ...
            <div
                id="chatOptions"
                className={`absolute bg-white dark:bg-blend-900 rounded-lg shadow-lg z-70 border border-gray-200 dark:border-blend-200 backdrop-blur-sm min-w-fit ${isOpen ? 'animate-enter' : 'animate-exit'}`}
                onClick={(e) => e.stopPropagation()}
            >
                <div className="p-2">
                    <button
                        onClick={() => globalEventBus.emit('rename:dialog:open', { closeContext: onClose, id: id })}
                        id="renameOption"
...
export const RenameDialog = ({ }) => {
    ...
    const handleKeyPress = (event) => {
        ...
        if (event.key === 'Enter' && !event.shiftKey && isOpen) {
            renameChatItem(name)
    ...
    useEffect(() => {
        const onOpen = globalEventBus.on('rename:dialog:open', ({ closeContext, id }) => {
            setIsOpen(true)
            setId(id)
            closeMenu = closeContext
        })
        return () => onOpen.unsubscribe()
    })

    const renameChatItem = useCallback(async () => {
        try {
            const newName = nameInput?.current.value?.trim()
            const rename = window.desk.api.RenameConversation(id, newName);
            if (rename) {
                globalEventBus.emit('chatitem:name:update', ({ id: id, name: newName }))
    ...
    return (
        <div
            className={`fixed inset-0 z-[60] flex items-center justify-center bg-black bg-opacity-40 transition ${isOpen ? 'animate-enter' : 'animate-exit'} duration-300 w-[100vw]`}
            onClick={(e) => {
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full mx-4">
                ...
                <div className="mb-4">
                    ...
                    <input
                        ...
                        type="text"
                        id="newName"
                        className="w-full p-2 border border-gray-400/70 rounded focus:ring-none focus:border-blue-500 focus:outline-none"
